...
echo %YELLOW%[2/10] Testing Password Policy - Weak Password...%RESET%
set /a TOTAL+=1
...
echo %YELLOW%[3/10] Testing User Registration...%RESET%
set /a TOTAL+=1
curl -s -X POST %BASE_URL%/auth/register ^
  -H "Content-Type: application/json" ^
...
echo %YELLOW%[4/10] Testing Authentication - Login...%RESET%
set /a TOTAL+=1
...
REM Extract token from JSON response (simple extraction)
set TOKEN1=
...
if defined TOKEN1 (
    if not "!TOKEN1!"=="" (
        echo %GREEN%✓ PASS%RESET% - Login successful, JWT received
        set /a PASSED+=1
    ) else (
        echo %RED%✗ FAIL%RESET% - Login returned 200 but no token
        set /a FAILED+=1
    )
) else (
    echo %RED%✗ FAIL%RESET% - Login failed or no token received
    type test-login1.tmp
    set /a FAILED+=1
)
echo.
...
echo %YELLOW%[6/10] Testing Note Creation...%RESET%
set /a TOTAL+=1
...
REM Extract note ID (simple extraction)
set NOTE1_ID=
...
echo %YELLOW%[8/10] Testing Data Isolation - Register User 2...%RESET%
set /a TOTAL+=1
curl -s -X POST %BASE_URL%/auth/register ^
  -H "Content-Type: application/json" ^
...
if defined TOKEN2 (
    if not "!TOKEN2!"=="" (
        echo %GREEN%✓ PASS%RESET% - User 2 registered and logged in
        set /a PASSED+=1
    ) else (
...
echo %YELLOW%[9/10] Testing IDOR - User 2 accessing User 1's Note...%RESET%
set /a TOTAL+=1
if not defined NOTE1_ID set NOTE1_ID=1
...
if "%STATUS%"=="404" (
    ...
    echo %RED%✗ FAIL%RESET% - IDOR vulnerability - User 2 accessed User 1's note (got %STATUS%^)
    set /a FAILED+=1
)
echo.
...
REM Verify notes table still exists by fetching notes
curl -s -X GET %BASE_URL%/api/notes ^
  -H "Authorization: Bearer %TOKEN1%" ^
...
if "%VERIFY_STATUS%"=="200" (
    echo %GREEN%✓ PASS%RESET% - SQL injection prevented - Database intact
    set /a PASSED+=1
) else (
    echo %RED%✗ FAIL%RESET% - SQL injection may have succeeded (got %VERIFY_STATUS%^)
    set /a FAILED+=1
)
echo.
