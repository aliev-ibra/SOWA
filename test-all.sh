...
# ========================================
echo -e "${YELLOW}[1/10] Testing Application Health Check...${RESET}"
((TOTAL++))
STATUS=$(curl -s -o /dev/null -w "%{http_code}" $BASE_URL/auth/login 2>/dev/null)
if [ "$STATUS" == "405" ]; then
    echo -e "${GREEN}✓ PASS${RESET} - Application is running (405 - route exists)"
    ((PASSED++))
else
...
# ========================================
echo -e "${YELLOW}[2/10] Testing Weak Password Rejection...${RESET}"
((TOTAL++))
...
# ========================================
echo -e "${YELLOW}[3/10] Testing Valid User Registration...${RESET}"
((TOTAL++))
STATUS=$(curl -s -X POST $BASE_URL/auth/register \
  -H "Content-Type: application/json" \
...
# ========================================
# Test 4: User Login & JWT Extraction
# ========================================
echo -e "${YELLOW}[4/10] Testing User Login & JWT Extraction...${RESET}"
((TOTAL++))
...
if [ ! -z "$TOKEN1" ] && [ "$TOKEN1" != "null" ] && [ "$TOKEN1" != "" ]; then
    echo -e "${GREEN}✓ PASS${RESET} - Login successful, JWT token extracted"
    ((PASSED++))
else
...
# ========================================
# Test 6: Create Note with Authentication
# ========================================
echo -e "${YELLOW}[6/10] Testing Create Note with Authentication...${RESET}"
((TOTAL++))
if [ -z "$TOKEN1" ]; then
    echo -e "${RED}✗ FAIL${RESET} - Cannot test: TOKEN1 not available"
    ((FAILED++))
else
    ...
    if [ ! -z "$NOTE1_ID" ] && [ "$NOTE1_ID" != "null" ] && [ "$NOTE1_ID" != "" ]; then
        ...
        echo -e "${RED}✗ FAIL${RESET} - Note creation failed or ID extraction failed"
        echo "$RESPONSE"
        ((FAILED++))
    fi
fi
...
# ========================================
# Test 7: Input Validation - Empty Title
# ========================================
...
# ========================================
echo -e "${YELLOW}[8/10] Testing Register Second User...${RESET}"
((TOTAL++))
...
if [ ! -z "$TOKEN2" ] && [ "$TOKEN2" != "null" ] && [ "$TOKEN2" != "" ]; then
    echo -e "${GREEN}✓ PASS${RESET} - User 2 registered and logged in, token extracted"
    ((PASSED++))
else
...
# ========================================
# Test 9: IDOR Prevention (Critical Security Test)
# ========================================
echo -e "${YELLOW}[9/10] Testing IDOR Prevention - User 2 accessing User 1's Note...${RESET}"
((TOTAL++))
if [ -z "$TOKEN2" ]; then
    echo -e "${RED}✗ FAIL${RESET} - Cannot test: TOKEN2 not available"
    ((FAILED++))
else
    if [ -z "$NOTE1_ID" ]; then
        NOTE1_ID=1
    ...
    if [ "$STATUS" == "404" ]; then
        ...
        echo -e "${RED}✗ FAIL${RESET} - IDOR vulnerability - User 2 accessed User 1's note (got $STATUS, expected 404)"
        ((FAILED++))
    fi
fi
echo
...
if [ -z "$TOKEN1" ]; then
    ...
    # Verify database is still intact by fetching notes
    STATUS=$(curl -s -X GET $BASE_URL/api/notes \
      -H "Authorization: Bearer $TOKEN1" \
      -w "%{http_code}" -o /dev/null 2>/dev/null)

    if [ "$STATUS" == "200" ]; then
        echo -e "${GREEN}✓ PASS${RESET} - SQL injection prevented - Database intact (200)"
        ((PASSED++))
    else
        echo -e "${RED}✗ FAIL${RESET} - SQL injection may have succeeded (got $STATUS, expected 200)"
        ((FAILED++))
    fi
fi
echo
